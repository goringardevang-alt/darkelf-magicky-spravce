...
// ─────────────────────────────────────────────────────────────
// Dark Elf - Magický správce v1.45   (Core Utils v2.33 uvnitř)
//
...
(function() {
    ...
    window.DarkElfUtils = {
        ...
        Cache: {
            ...
            set: function(namespace, identifier, value, expiresAt) {
                ...
                try {
                    ...
                    this._uklidJednouDenne();
                    const text = JSON.stringify({
                        data: value,
                        expires: expiresAt || this._getMidnight()
                    });
                    if (this._zapis(key, text)) return true;
                    this._uklidProsle();
                    if (this._zapis(key, text)) return true;
                    this._uklidCiziLigy();
                    if (this._zapis(key, text)) return true;
                    console.warn('DarkElfUtils.Cache: úložiště je plné, ' + key + ' se neuložil');
                    return false;
                } catch (e) {
                    console.warn('DarkElfUtils.Cache.set:', e && e.message);
                    return false;
                }
            },

            _zapis: function(key, text) {
            ...
            _klice: function() {
                ...
                for (let i = 0; i < localStorage.length; i++) {
                    ...
                    if (k && k.indexOf('DE_CACHE_') === 0) out.push(k);
                }
                return out;
            },

            _uklidProsle: function() {
                const ted = Date.now();
                let smazano = 0;
                this._klice().forEach(k => {
                    const m = String(localStorage.getItem(k) || '').match(/"expires":(\d+)\D*$/);
                    if (m && parseInt(m[1], 10) < ted) { localStorage.removeItem(k); smazano++; }
                });
                return smazano;
            },

            _uklidCiziLigy: function() {
                const liga = window.DarkElfUtils.getLeague() || 'ALL';
                const vlastni = 'DE_CACHE_' + liga + '_';
                let smazano = 0;
                this._klice().forEach(k => {
                    if (k.indexOf(vlastni) === 0) return;
                    if (!/^DE_CACHE_[^_]+_api_/.test(k)) return;
            ...
            _uklidJednouDenne: function() {
                const d = new Date();
                const dnes = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
                try {
                    if (localStorage.getItem('de_cache_uklid') === dnes) return;
                    this._uklidProsle();
                    localStorage.setItem('de_cache_uklid', dnes);
                } catch (e) {  }
            },
        ...
        MapAPI: {
            ...
            fetch: async function(area, force) {
                ...
                if (!this._patriPrihlasenemu(cached)) {
                    return this._nahradCizi(area);
                }
                ...
                const vysledek = await this._stahni(area);
                return vysledek.data || cached || null;
            },
            ...
            getMyRace: function() {
                ...
                if (!this._patriPrihlasenemu(cached)) return null;
            ...
            getMe: function() {
                const cached = window.DarkElfUtils.Cache.get('api', 'map_json');
                const stav = this._stavSnimku(cached);
                if (stav !== 'ok' && stav !== 'neznamo') return null;
                const h = cached.hlavicka;
                if (h.id_hrace == null && !h.hrac) return null;
                return {
                    id: h.id_hrace == null ? null : String(h.id_hrace),
                    jmeno: h.hrac || null,
                    id_aliance: h.id_aliance == null ? null : parseInt(h.id_aliance, 10),
                    overeno: stav === 'ok'
                };
            },
            ...
            isStale: function() {
                ...
                if (!this._patriPrihlasenemu(cached)) return true;
                return this._jeNovySnimek(cached, Date.now());
            },

            _patriPrihlasenemu: function(snimek) {
                const stav = this._stavSnimku(snimek);
                return stav !== 'cizi' && stav !== 'predVstupem';
            },

            _stavSnimku: function(snimek) {
                const h = snimek && snimek.hlavicka;
                if (!h) return 'zadny';
                const P = window.DarkElfUtils.Prihlaseni;
                const p = P ? P.precti() : null;
                if (!p) return 'neznamo';
                const kdy = snimek._fetchedAt || 0;
                const idSnimku = String(h.id_hrace == null ? '' : h.id_hrace);
                if (p.id) {
                    if (idSnimku === p.id) return 'ok';
                    return kdy < (p.ts || p.tsId || 0) ? 'cizi' : 'ok';
                }
                if (p.ciziId && idSnimku === p.ciziId && kdy < (p.ts || p.tsCizi || 0)) return 'cizi';
                if (p.ts) return kdy < p.ts ? 'predVstupem' : 'ok';
                return 'neznamo';
            },

            _stahni: async function(area) {
                const now = Date.now();
                ...
                try {
                    ...
                    if (data.error) {
                        ...
                        return { chyba: data.error, retryAfter: data.retry_after };
                    }
            ...
            _nahradCizi: function(area) {
                return window.DarkElfUtils.Prihlaseni._nahradSnimek(this, 'map_json', () => this._stahni(area));
            }
        },
        ...
        TreatiesAPI: {
            ...
            fetch: async function(force) {
                ...
                if (!this._patriPrihlasenemu(cached)) {
                    return window.DarkElfUtils.Prihlaseni._nahradSnimek(this, 'treaties_json', () => this._stahni());
                }
                ...
                const vysledek = await this._stahni();
                return vysledek.data || cached || null;
            },

            _stahni: async function() {
                const now = Date.now();
                ...
                try {
                    ...
                    if (data.error) {
                        console.warn('DarkElfUtils.TreatiesAPI:', data.error, 'retry_after:', data.retry_after);
                        return { chyba: data.error, retryAfter: data.retry_after };
                    }
            ...
            _patriPrihlasenemu: function(snimek) {
                return window.DarkElfUtils.MapAPI._patriPrihlasenemu(snimek);
            },
            ...
            getExpiring: function(countryId) {
                ...
                if (!this._patriPrihlasenemu(window.DarkElfUtils.Cache.get('api', 'treaties_json'))) return out;
                const entry = this.getCountry(countryId);
            ...
            getMilitaryAllianceDefence: function(countryId) {
                if (!this._patriPrihlasenemu(window.DarkElfUtils.Cache.get('api', 'treaties_json'))) return 0;
                const entry = this.getCountry(countryId);
    ...
    window.DarkElfUtils.AllianceAPI = {
        ...
        getMembers: function() {
            ...
            if (!window.DarkElfUtils.MapAPI._patriPrihlasenemu(cached)) return [];
    ...
    window.DarkElfUtils.Prihlaseni = {
        _KLIC: 'de_prihlaseni',

        precti: function() {
            try { return JSON.parse(localStorage.getItem(this._KLIC)) || null; }
            catch (e) { return null; }
        },

        _zapis: function(stav) {
            try { localStorage.setItem(this._KLIC, JSON.stringify(stav)); }
            catch (e) { return; }
            this._ohlas(stav);
        },

        _ohlas: function(stav) {
            try { window.dispatchEvent(new CustomEvent('de-prihlaseni', { detail: stav || this.precti() })); }
            catch (e) {  }
        },

        vstupDoHry: function() {
            this._zapis({ ts: Date.now() });
        },

        ohlasHrace: function(id, zdroj) {
            if (id == null || id === '') return;
            id = String(id);
            const p = this.precti() || {};
            if (p.id !== id) {
                this._zapis({ ts: p.ts, id: id, tsId: Date.now(), zdroj: zdroj });
            }
            this._nahradJeCizi();
        },

        ohlasCiziSnimek: function(ciziId, zdroj) {
            const p = this.precti() || {};
            if (p.id) return;
            ciziId = String(ciziId);
            if (p.ciziId !== ciziId) {
                this._zapis(Object.assign({}, p, { ciziId: ciziId, tsCizi: Date.now(), zdroj: zdroj }));
            }
            this._nahradJeCizi();
        },

        _nahradJeCizi: function() {
            const M = window.DarkElfUtils.MapAPI;
            if (M._stavSnimku(window.DarkElfUtils.Cache.get('api', 'map_json')) === 'cizi') {
                M.fetch().catch(function() {  });
            }

        },

        _nahradSnimek: function(api, klic, stahni) {
            if (api._nahrazovani) return api._nahrazovani;
            const cekej = (ms) => new Promise(r => setTimeout(r, ms));
            const POKUSU = 3;
            api._nahrazovani = (async () => {
                try {
                    for (let pokus = 1; pokus <= POKUSU; pokus++) {
                        const zbyva = api._RATE_LIMIT_MS - (Date.now() - api._lastFetchTime);
                        if (zbyva > 0) await cekej(zbyva);

                        const cached = window.DarkElfUtils.Cache.get('api', klic);
                        if (cached && api._patriPrihlasenemu(cached)) return cached;

                        const v = await stahni();
                        if (v.data) return v.data;
                        if (!v.chyba) break;
                        if (pokus < POKUSU) {
                            const s = Number(v.retryAfter);
                            await cekej((Number.isFinite(s) && s > 0 ? s : 1) * 1000 + Math.random() * 500);
                        }
                    }
                    console.warn('DarkElfUtils: snímek ' + klic + ' předchozího hráče se nepodařilo nahradit, vracím null');
                    return null;
                } finally {
                    api._nahrazovani = null;
                }
            })();
            return api._nahrazovani;
        },

        mojeZemNaMape: function(doc) {
            const lands = (doc || document).querySelectorAll('.land[data-id_player]');
            for (const land of lands) {
                const img = land.querySelector('img[src*="images/v/"]');
                if (img && /\/v\/\d+a\.gif$/i.test(img.getAttribute('src') || '')) {
                    const hrac = land.getAttribute('data-id_player');
                    if (hrac) return { id: parseInt(land.getAttribute('data-id'), 10), hrac: hrac };
                }
            }
            return null;
        },

        zemeZCentraly: function(doc) {
            const ids = [];
            (doc || document).querySelectorAll('td.pen').forEach(function(td) {
                const tr = td.closest('tr');
                const a = tr && tr.querySelector('a[href*="e.asp?id="]');
                const m = a && (a.getAttribute('href') || '').match(/e\.asp\?id=(\d+)/);
                if (m) ids.push(parseInt(m[1], 10));
            });
            return ids;
        },

        _overCentralu: function(doc) {
            const ids = this.zemeZCentraly(doc);
            const snimek = window.DarkElfUtils.Cache.get('api', 'map_json');
            if (!ids.length || !snimek || !snimek.hlavicka || !Array.isArray(snimek.zeme)) return;
            const idSnimku = String(snimek.hlavicka.id_hrace == null ? '' : snimek.hlavicka.id_hrace);
            if (!idSnimku) return;
            let vSnimku = 0, sedi = 0;
            ids.forEach(function(id) {
                const z = snimek.zeme.find(function(x) { return x.id === id; });
                if (!z) return;
                vSnimku++;
                if (String(z.id_hrac) === idSnimku) sedi++;
            });
            if (!vSnimku) return;
            if (sedi * 2 > vSnimku) this.ohlasHrace(idSnimku, 'centrala');
            else if (sedi === 0) this.ohlasCiziSnimek(idSnimku, 'centrala');

        },

        _priNacteni: function() {
            const cesta = (window.location.pathname || '').toLowerCase();
            if (cesta === '/world.htm') { this.vstupDoHry(); return; }

            const self = this;
            window.addEventListener('storage', function(e) {
                if (e.key === self._KLIC) self._ohlas();
            });

            const moje = this.mojeZemNaMape(document);
            if (moje) this.ohlasHrace(moje.hrac, 'mapa');
            else if (cesta === '/centrala.asp') this._overCentralu(document);
        }
    };
    ...
    try { window.DarkElfUtils.Prihlaseni._priNacteni(); }
    catch (e) { console.warn('DarkElfUtils.Prihlaseni:', e); }
...
(function() {
    ...
    const ZEME_NASOBEK_RE = /^\s*(\d+)\s*(?:[×xX*](\++)?\s+|(\++)\s*[×xX*]\s*)/;
    ...
    function kouzloSNasobkem(text) {
        ...
        if (k && !k.zbytekRaw) return { kouzlo: k, nasobek: s.nasobek, plusy: s.plusy };

        const holeCislo = String(text || "").match(/^\s*(\d+)(\+*)\s+(.+)$/);
        if (holeCislo) {
            k = window.DarkElfUtils.Spells.byName(holeCislo[3]);
            if (k && !k.zbytekRaw) {
                return { kouzlo: k, nasobek: parseInt(holeCislo[1], 10),
                         plusy: holeCislo[2].length };
            }
        }
        return null;
    }

    const MO_ZA_KOUZLEM = /^[\s\-–—]*\(?\s*(mo\s*-?\s*\d+(?:\s*-\s*\d+)?\s*\+?)\s*\??\s*\)?\s*/i;
    ...
    function vypadaJakoNovyRadek(line, zemeIdx) {
        ...
        if (/\d\s*[-–]\s*\d/.test(String(line || ""))) return true;

        if (zemeIdx) {
            const prvni = String(line || "").split(ZEME_ODDELOVAC)[0]
                .replace(ZEME_NASOBEK_RE, " ").replace(/\([^)]*\)?/g, " ").trim();
            if (prvni && !zemeIdx[normalizeText(prvni)]
                && prvni.split(/[\s_()]+/).some(slovo => {
                    if (slovo.length < 2) return false;
                    const k = window.DarkElfUtils.Spells.byName(slovo);
                    return !!k && !k.zbytekRaw;
                })) {
                return true;
            }
        }
    ...
    const POZNAMKA_UVOZ_RE = /["„“”]([^"„“”]*)["„“”]/g;

    function vytahniUvozovky(text) {
        const poznamky = [];
        const bez = String(text || "").replace(POZNAMKA_UVOZ_RE, (m, t) => {
            if (t.trim()) poznamky.push(t.trim());
            return " ";
        }).replace(/\s+/g, " ").trim();
        return { text: bez, poznamka: poznamky.join(" ") };
    }
    ...
    function vytahniNasobek(stitek) {
        const t0 = String(stitek || "");

        const rozsah = rozsahNasobku(t0);
        if (rozsah) {
            const vnitrek = vytahniNasobek(rozsah.zbytek);
            if (vnitrek.nasobek === 1 && !vnitrek.plusy) {
                return { nasobek: rozsah.nasobek, plusy: rozsah.plusy, zbytek: vnitrek.zbytek };
            }
        }
        const t = t0;
        ...
        const hole = t.match(/^\s*(\d+)(\++)?\s+(.+)$/);
        if (hole && window.DarkElfUtils.Spells.byName(hole[3])) {
            return { nasobek: parseInt(hole[1], 10), plusy: (hole[2] || "").length,
                     zbytek: hole[3] };
        }
        return { nasobek: 1, plusy: 0, zbytek: t };
    }

    function rozsahNasobku(text) {
        const t = String(text || "");
        const m = t.match(/^\s*(\d)\s*[-–]\s*(\d)\s*[×xX*]?\s*(?=\S)/)
               || t.match(/\(\s*(\d)\s*[-–]\s*(\d)\s*\)/);
        if (!m) return null;
        const od = parseInt(m[1], 10), po = parseInt(m[2], 10);
        if (od < 1 || po <= od) return null;
        const zbytek = (t.slice(0, m.index) + " " + t.slice(m.index + m[0].length))
            .replace(/\s+/g, " ").trim();
        if (!zbytek) return null;
        return { nasobek: od, plusy: po - od, zbytek: zbytek };
    }
    ...
    const PRIZNAK_SKMIN_RE = /(^|[\s_])sk\s*(\d+)(?:\s*-\s*\d+)?\s*\+?(?=$|[\s_])/i;

    function skMinZeStitku(stitek) {
        const m = String(stitek || "").match(PRIZNAK_SKMIN_RE);
    ...
    function slucRadky(radky) {
        ...
        radky.forEach(r => {
            ...
            const klic = [r.kat || "", r.kouzlo.nazev, r.mo, r.poznamka || "",
                          r.poznamkaNejista || "",
            ...
            if (!podleKlice[klic]) {
                ...
                podleKlice[klic] = kopieRadku(r, { zeme: [], videno: [], poznamky: [], moZeme: {} });
                poradi.push(klic);
            }
            const cil = podleKlice[klic];
            if (r.poznamka && cil.poznamky.indexOf(r.poznamka) === -1) cil.poznamky.push(r.poznamka);

            Object.keys(r.moZeme || {}).forEach(k => {
                const nova = r.moZeme[k], stara = cil.moZeme[k];
                if (!stara) { cil.moZeme[k] = nova; return; }
                const a = moRozsah(stara), b = moRozsah(nova);
                cil.moZeme[k] = (b && (!a || b.min > a.min)) ? nova : stara;
            });
            r.zeme.forEach(z => {
    ...
    const HRAC_RADEK_RE = /^(.+?)\s+-\s+(\d+)\s+-\s+\/([^\/]*)\/\s*-\s*(.*)$/;
    ...
    function hracskyRadek(line) {
        const m = String(line || "").match(HRAC_RADEK_RE);
        if (!m) return null;

        if (m[1].indexOf(":") > -1) return null;
        const casti = String(m[4] || "").split(HRAC_DOPISEK_ODDELOVAC)
                          .map(x => x.trim()).filter(Boolean);
        return { jmeno: m[1].trim(), pocet: parseInt(m[2], 10),
                 mimo: parseLands(m[3]),
                 poznamka: casti.filter(x => !jeNasDopisek(x)).join(" · "),
                 dopisky: casti.filter(jeNasDopisek) };
    }

    function zapisHrace(h, dopisky) {
        const mimo = (h.mimo || []).length ? ("/" + h.mimo.join(",") + "/") : "//";
        const nas = dopisky
            ? (dopisky[normalizeText(h.jmeno)] || "")
            : (h.dopisky || []).join(" · ");
        const pozn = [h.poznamka, nas].filter(Boolean).join(" · ");
        return [h.jmeno, h.pocet, mimo, pozn].join(" - ").replace(/\s+$/, "");
    }
    ...
    function katZeSekce(nazev) {
        ...
        if (/(^|[\s_\-])zakouzl/.test(n)) return KAT_ZAKOUZLENO;
        if (/(^|[\s_\-])jistot/.test(n)) return KAT_JISTOTA;
        ...
        if (m) {
            ...
            if (c < 1) return KAT_TOP;
            return "Prio " + Math.min(c, 3);
        }
    ...
    function posudNaJmeno(text, ctx, majitel, ptatSe) {
        ...
        const zeptejSe = (kandidati) => {
            if (!ptatSe || !ctx || !ctx.nejista) return false;
            if (!ctx.nejista.some(x => x.klic === klic)) {
        ...
        if (!hraci.length) return { nejiste: zeptejSe([]) };
        ...
        return { nejiste: zeptejSe(kandidati) };
    }
    ...
    function rozdelHraceAPoznamku(zbytek, uvnitrHracskeSekce, ctx, majitel, ptatSe) {
        ...
        return { hrac: "", poznamka: t, nejiste: !!soud.nejiste };
    }
    ...
    function doplnNeutralky(sekce, ctx) {
        ...
        sekce.forEach(sek => {
            ...
            sek.radky.forEach(r => {
                ...
                if (r.mo || r.neu || r.skmax || r.skMin || r.kat === KAT_ZAKOUZLENO
                    || !r.zeme.length) { nove.push(r); return; }
    ...
    function parseML(text, ctx) {
        ...
        String(text || "").split(/\r?\n/).forEach(raw => {
            ...
            const uvoz = vytahniUvozovky(surovyStitek);
            const stitek = uvoz.text.replace(ZEME_POKROK_RE, " ")
                .replace(HOTOVO_RE, " ")
                .replace(/\s+/g, " ").trim();
            ...
            if (spojena) {
                ...
                spojena.forEach(k => {
                    aktualni.radky.push({
                        ...
                        mo: moStitu, poznamka: uvoz.poznamka,
            ...
            if (stitek && nepoznaneSpojeni(zbytek)) {
            ...
            if (varianty) {
                aktualni.radky.push({
                    ...
                    mo: vytahniMO(stitek), poznamka: uvoz.poznamka,
                    zeme: parseLands(telo), moZeme: moZeZavorek(telo),
            ...
            if (kouzlo) {
                ...
                if (posledni.kat === KAT_ZAKOUZLENO) {

                    const kdo = rozdel.hrac
                        ? (rozdel.poznamka ? (rozdel.hrac + "= " + rozdel.poznamka) : rozdel.hrac)
                        : rozdel.poznamka;
                    posledni.poznamka = [kdo, uvoz.poznamka ? ('"' + uvoz.poznamka + '"') : ""]
                        .filter(Boolean).join(" ");
                    return;
                }

                posledni.poznamkaNejista = rozdel.nejiste ? rozdel.poznamka : "";
                posledni.poznamka = [rozdel.nejiste ? "" : rozdel.poznamka, uvoz.poznamka]
                    .filter(Boolean).join(" ");
                return;
            }
            ...
            if (!maDvojtecku) {
                const uvozR = vytahniUvozovky(line);
                const rozpad = kouzlaAPakZeme(uvozR.text);
            ...
            if (!maDvojtecku && posledni) {
                ...
                const dalsiNasobky = nasobkyZeme(line);
                if (Object.keys(dalsiNasobky).length) {
                    posledni.nasobekZeme = Object.assign(posledni.nasobekZeme || {}, dalsiNasobky);
                }
                return;
            }
    ...
    function nepoznaneSpojeni(stitek) {
        const S = window.DarkElfUtils.Spells;
        const t = String(stitek || "").replace(/mo\s*-?\s*\d+(?:\s*-\s*\d+)?\s*\+?/ig, " ");
        const m = t.match(/\p{L}\s*\+\s*(\p{L}+)/u);
        if (!m) return false;
        const slovo = S.norm(m[1]);
        if (slovo.length < 3) return false;
        return Object.keys(S.LIST).concat(Object.keys(S.ALIAS))
            .some(x => S.norm(x).indexOf(slovo) === 0);
    }
    ...
    function spojRodiny(radky) {
        ...
        const spojitelny = (r) => !!rodinaKouzla(r.kouzlo.nazev) && r.nasobek === 1
            && !(r.varianty && r.varianty.length);

        const klicem = (r) => [rodinaKouzla(r.kouzlo.nazev), r.mo || "",
                               r.prio ? "!" : "", r.skmax ? "skmax" : "", r.skMin || "",
                               r.plusy || 0, r.hotovo || 0, r.neu ? "neu" : "",
                               r.poznamka || "", r.poznamkaNejista || "",
                               r.zeme.join("")].join("|");
    ...
    function rozpadKose(radky) {
        ...
        radky.forEach(r => {
            ...
            const kopie = (mo, zeme) => {
                ...
                return kopieRadku(r, { mo: mo, zeme: zeme, moZeme: moZeme });
            };
    ...
    function slucDoRadku(radky, mo) {
        ...
        return kopieRadku(prvni, { mo: mo, zeme: zeme, moZeme: moZeme });
    }
    ...
    function seskupPoKosich(vstup) {
        ...
        radky.forEach(r => {
            ...
            const klic = [r.kouzlo.nazev, r.poznamka || "", r.poznamkaNejista || "",
                          r.nasobek, r.prio ? "!" : "",
                          r.skmax ? "skmax" : "", r.skMin || "",
                          variantyRadku(r).join("/"), r.plusy || 0, r.hotovo || 0,
                          kos].join("|");
        ...
        poradi.forEach(klic => {
            ...
            const jePresna = (r) => { const q = moRozsah(r.mo); return !!q && isFinite(q.max); };

            const zemiHodnoty = {};
            sk.radky.forEach(r => {
                if (jePresna(r)) zemiHodnoty[r.mo] = (zemiHodnoty[r.mo] || 0) + r.zeme.length;
            });

            const skupinky = [], kdeHodnota = {}, zbytek = [];
            sk.radky.forEach(r => {
                if (jePresna(r) && (zemiHodnoty[r.mo] || 0) >= PRAH_SESKUPENI) {
                    if (kdeHodnota[r.mo] == null) {
                        kdeHodnota[r.mo] = skupinky.length;
                        skupinky.push({ mo: r.mo, radky: [] });
                    }
                    skupinky[kdeHodnota[r.mo]].radky.push(r);
                } else zbytek.push(r);
            });
            if (zbytek.length) skupinky.push({ mo: null, radky: zbytek });

            skupinky.forEach(sub => {
                const radky = sub.radky;

                if (sub.mo) { out.push(slucDoRadku(radky, sub.mo)); return; }

                const zbyloSeslani = radky.reduce((n, r) => n + r.zeme.length, 0);
                if (radky.length < 2 || zbyloSeslani < PRAH_SESKUPENI) {
                    radky.forEach(r => out.push(r));
                    return;
                }

                out.push(slucDoRadku(radky, "MO" + sk.kos + "+"));
            });
        });
    ...
    function rozpadPodleMO(radky) {
        ...
        (radky || []).forEach(r => {
            ...
            poradi.forEach(mo => {
                ...
                out.push(kopieRadku(r, { mo: mo, zeme: skupiny[mo], moZeme: moZeme }));
            });
        });
        return out;
    }

    function exportML(data, cenaSouhrn, dnes, pokrok, dopisky) {
        const den = dnes || new Date();
        const hlavicka = "ML " + den.getDate() + "." + (den.getMonth() + 1) + ".";
        const radky = slozML(data, cenaSouhrn, pokrok, dopisky).split("\n");
    ...
    function rozdelPodlePokroku(r, pokrok) {
        if (!pokrok || !(r.nasobek > 1) || !r.zeme.length) return [{ radek: r, hotovo: 0 }];
        const skupiny = {}, poradi = [];
        r.zeme.forEach(z => {
            const h = pokrok[klicPokroku(r, z)] || 0;
            if (!skupiny[h]) { skupiny[h] = []; poradi.push(h); }
            skupiny[h].push(z);
        });
        if (poradi.length === 1 && poradi[0] === 0) return [{ radek: r, hotovo: 0 }];

        poradi.sort((a, b) => a - b);
        return poradi.map(h => ({
            radek: Object.assign({}, r, { zeme: skupiny[h] }),
            hotovo: h
        }));
    }

    function slozML(data, cenaSouhrn, pokrok, dopisky) {
        const radky = [];
        if (data.hlavicka) radky.push(data.hlavicka.text);

        if ((data.hraci || []).length) {
            if (radky.length) radky.push("");
            data.hraci.forEach(h => radky.push(zapisHrace(h, dopisky)));
        }
        let prvniSekce = true;
        poradiProVystup(data).forEach(sek => {
            ...
            let minulaKat = null;
            spojRodiny(seskupPoKosich(sek.radky)).forEach(puvodni => {
                const kat = skupinaKouzla(puvodni).skupina;
                ...
                if (r.poznamkaNejista) stitek += "_" + r.poznamkaNejista;

                if (r.poznamka) {
                    stitek += (r.kat === KAT_ZAKOUZLENO)
                        ? ("_" + r.poznamka) : (' "' + r.poznamka + '"');
                }
    ...
    function moZeme(z, den) {
        ...
        if (N && N.jeNeutralka(z)) {
            ...
            return { jistota: "presna", mo: v.mo, vez: v.vez, zdroj: "neutralka",
    ...
    function chybyZapisu(data, idx, text, cast, vyresene) {
            ...
            kontrolaOs(data, cast, idx).forEach(n => {
                ...
                nalezy.push({
                    ...
                    popis: "letí " + n.zdroje.join(", ") + ", list chce " + zkr
                           + " — co je omyl? (" + dovetekKonfliktu(n, zkr) + ")",
                    radek: k.radek, textRadku: k.textRadku, od: k.od, do: k.do,
                    odVRadku: k.odVRadku, doVRadku: k.doVRadku });
            });
        }
        ...
        if (idx) {
            kontrolaOsVListu(data, idx).forEach(n => {
                const kde = najdiVText(text, n.zeme);
                const k = (kde.length ? kde : nenalezeno(n.zeme))[0];
                nalezy.push({
                    druh: "osaList", cast: n.zeme, osaNalez: n, popis: n.popis,
                    radek: k.radek, textRadku: k.textRadku, od: k.od, do: k.do,
                    odVRadku: k.odVRadku, doVRadku: k.doVRadku });
            });
        }
    ...
    function kroku(n) {
        return n + " " + (n === 1 ? "krok" : (n >= 2 && n <= 4) ? "kroky" : "kroků");
    }

    function dovetekKonfliktu(n, zkr) {
        if (n.vysledek === "nic") {
            return "žlutou to nepřebiješ, letí " + kroku(n.dolu) + " dolů";
        }
        if (n.vysledek === "neutral") return zkr + " to srazí jen na neutrál";
        const nyni = (n.radek && n.radek.nasobek) || 1;
        return (Math.abs(n.zbyva) > nyni) ? ("jinak " + Math.abs(n.zbyva) + "×") : "stačí, jak je psáno";
    }
    ...
    function navrhyZakouzleno(data, cast, idx, volby) {
        ...
        jizZakouzlenoVPlanu(data, cast, idx).forEach(n => {
            ...
            const padaNiz = !!n.radek.plusy && !v.slaboProSKmax;
            const kam = padaNiz
                ? nizsiKategorie(n.radek.kat)
                : (v.slaboProSKmax ? KAT_JISTOTA : KAT_ZAKOUZLENO);
            ...
            out.push({ zeme: n.zeme, id: n.id, kouzlo: n.kouzlo, sila: n.sila, kouzlic: n.kouzlic,
                       ...
                       popis: "síla " + n.sila + (pozadavek > 0 ? (" na MO" + pozadavek) : "") });
        });
        return out;
    }
    ...
    function vyhodnotSeslani(n, idx, prahSK, volby) {
        ...
        return { typ: typ, pozadavek: pozadavek, stav: stav,
                 ...
                 slaboProSKmin: slaboProSKmin,
                 splnil: projdeObranou(n.sila, pozadavek, typ) && !slaboProSKmin,
    ...
    function dodaneKroky(cast, zemeId, pozadavek) {
        ...
        const out = { osy: { porodnost: 0, zlato: 0, mana: 0 }, drzi: {}, leci: {}, zdroje: [],
                      ...
                      nahoru: { porodnost: 0, zlato: 0, mana: 0 },
                      dolu: { porodnost: 0, zlato: 0, mana: 0 } };
        ...
        ((cast && cast.kouzla) || []).forEach(k => {
            ...
            if (kroky) {
                Object.keys(kroky).forEach(osa => {
                    out.osy[osa] += kroky[osa];
                    if (kroky[osa] > 0) out.nahoru[osa] += kroky[osa];
                    else out.dolu[osa] -= kroky[osa];
                });
            ...
            r.zeme.forEach(jmeno => {
                ...
                Object.keys(kroky).forEach(osa => {
                    ...
                    if ((jizDodano > 0) !== (pozadovano > 0)) {
                        const naSeslani = Math.abs(kroky[osa]) || 1;
                        let zbyva, vysledek;
                        if (pozadovano < 0) {

                            const kroku = Math.max(Math.abs(pozadovano), 2) - dodano.dolu[osa];
                            zbyva = -Math.max(0, Math.ceil(kroku / naSeslani));
                            vysledek = "dno";
                        } else {

                            const dolu = dodano.dolu[osa];
                            zbyva = (dolu <= 1) ? 1 : 0;
                            vysledek = (dolu <= 1) ? "neutral" : "nic";
                        }
                        nalezy.push({ zeme: jmeno, id: z.id, kouzlo: r.kouzlo.nazev, osa: osa,
                                      druh: "konflikt", pozadovano: pozadovano,
                                      dodano: jizDodano, zdroje: dodano.zdroje.slice(),
                                      dolu: dodano.dolu[osa], vysledek: vysledek,
                                      zbyva: zbyva, radek: r, sekce: sek,
                                      popis: "na zem letí " + dodano.zdroje.join(", ")
                                             + " — táhne " + osa + " opačně" });
                        return;
                    }

                    const zbyvaKroku = pozadovano - jizDodano;
                    const uzStaci = (pozadovano > 0) ? (zbyvaKroku <= 0) : (zbyvaKroku >= 0);
                    const naSeslani = Math.abs(kroky[osa]) || 1;
                    const zbyvaSeslani = Math.sign(zbyvaKroku)
                        * Math.ceil(Math.abs(zbyvaKroku) / naSeslani);
                    if (uzStaci || Math.abs(zbyvaSeslani) < (r.nasobek || 1)) {
                        nalezy.push({ zeme: jmeno, id: z.id, kouzlo: r.kouzlo.nazev, osa: osa,
                                      druh: uzStaci ? "netreba" : "mene",
                                      pozadovano: pozadovano, dodano: jizDodano,
                                      zdroje: dodano.zdroje.slice(),
                                      zbyva: uzStaci ? 0 : zbyvaSeslani, radek: r, sekce: sek,
    ...
    const SKALY_JEDNA_OSA = { "-1": { porodnost: "Nespokojenost", zlato: "Krupobití", mana: "Magický vír" }, "1": { porodnost: "Spokojenost", zlato: "Příznivé počasí", mana: "Magické klima" } };

    function zbyvaKrokuNaOsach(r, jmeno, cast, idx) {
        const S = window.DarkElfUtils.Spells;
        const kroky = r && r.kouzlo ? S.kroky(r.kouzlo.nazev) : null;
        const z = (kroky && idx) ? idx[normalizeText(jmeno)] : null;
        if (!z) return null;
        const pozadavek = (pozadavekZeme(r, jmeno) || { min: 0 }).min;
        const dodano = dodaneKroky(cast, z.id, pozadavek);
        const out = {};
        let smer = 0;
        const osy = Object.keys(kroky);
        for (let i = 0; i < osy.length; i++) {
            const osa = osy[i];
            if (dodano.drzi[osa]) return null;
            const chce = kroky[osa] * (r.nasobek || 1);
            const uz = dodano.osy[osa] || 0;
            if (uz && (uz > 0) !== (chce > 0)) return null;
            smer = Math.sign(chce);

            out[osa] = (dodano.leci[osa] && chce < 0) ? 0 : Math.max(0, Math.abs(chce) - Math.abs(uz));
        }
        return { smer: smer, osy: out };
    }

    function nahradniKouzla(r, zbyva) {
        const S = window.DarkElfUtils.Spells;
        if (!r || !zbyva || !zbyva.smer) return [];
        const osy = Object.keys(zbyva.osy);
        if (osy.length < 2) return [];
        const hodnoty = osy.map(o => zbyva.osy[o]);
        const max = Math.max.apply(null, hodnoty);
        if (max === 0) return [];

        const jedna = SKALY_JEDNA_OSA[String(zbyva.smer)];
        const cele = (zbyva.smer < 0) ? "Kletba" : "Požehnání";
        const out = [];
        const pridej = (kouzla) => {
            const k = kouzla.filter(x => x.nasobek > 0);
            if (!k.length) return;
            if (k.length === 1 && k[0].nazev === r.kouzlo.nazev
                && k[0].nasobek === (r.nasobek || 1)) return;
            const klic = k.map(x => x.nasobek + "×" + x.nazev).join("+");
            if (out.some(m => m.klic === klic)) return;
            out.push({ klic: klic, kouzla: k,
                       seslani: k.reduce((a, x) => a + x.nasobek, 0),
                       cena: k.reduce((a, x) => a + ((S.LIST[x.nazev] || {}).cena || 0) * x.nasobek, 0) });
        };

        pridej(osy.map(o => ({ nazev: jedna[o], nasobek: zbyva.osy[o] })));
        pridej([{ nazev: cele, nasobek: 1 }]
            .concat(osy.map(o => ({ nazev: jedna[o], nasobek: Math.max(0, zbyva.osy[o] - 1) }))));
        if (zbyva.smer < 0 && max >= 2) pridej([{ nazev: "Dvojitá kletba", nasobek: Math.ceil(max / 2) }]);
        else pridej([{ nazev: cele, nasobek: max }]);

        return out.sort((a, b) => (a.cena - b.cena) || (a.seslani - b.seslani));
    }

    function nahradKouzloNaZemi(radek, sekce, jmeno, kouzla) {
        const S = window.DarkElfUtils.Spells;
        const nove = (kouzla || []).map(k => radekProZem(radek, jmeno,
            { kouzlo: S.byName(k.nazev), nasobek: k.nasobek, varianty: null }));
        const jm = normalizeText(jmeno);
        radek.zeme = radek.zeme.filter(z => normalizeText(z) !== jm);
        if (radek.moZeme) delete radek.moZeme[jm];
        nove.forEach(n => sekce.radky.push(n));
        sekce.radky = slucRadky(sekce.radky.filter(x => x.zeme.length || x !== radek));
    }
    ...
    function kontrolaOsVListu(data, idx) {
        ...
        ((data && data.sekce) || []).forEach(sek => sek.radky.forEach(r => {
            ...
            r.zeme.forEach(jmeno => {
                const z = idx[normalizeText(jmeno)];
                if (!z) return;
                const zaznam = zeme[z.id] || (zeme[z.id] = { jmeno: jmeno, osy: {} });

                Object.keys(kroky).forEach(osa => {
                    const o = zaznam.osy[osa]
                        || (zaznam.osy[osa] = { nahoru: 0, dolu: 0, zdroje: [], radky: [], podleKat: {} });

                    if (kroky[osa] > 0) o.nahoru++; else o.dolu++;

                    const kat = r.kat || katRadku(sek.nazev, r.prio, "");
                    const smer = kroky[osa] > 0 ? "+" : "-";
                    o.podleKat[kat + smer] = (o.podleKat[kat + smer] || 0) + 1;
        ...
        Object.keys(zeme).forEach(id => {
            ...
            Object.keys(z.osy).forEach(osa => {
                ...
                else if (Object.keys(o.podleKat).some(k => o.podleKat[k] > 1)) dvakrat.push(osa);
                else return;
    ...
    function neprosleVPlanu(data, cast, idx, volby) {
        ...
        jizZakouzlenoVPlanu(data, cast, idx).forEach(n => {
            ...
            out.push({ zeme: n.zeme, id: n.id, kouzlo: n.kouzlo, sila: n.sila,
                       ...
                       skMin: v.slaboProSKmin ? n.radek.skMin : 0,
    ...
    function kopieRadku(zdroj, zmeny) {
        const z = zmeny || {};
        const platne = {};
        Object.keys(z).forEach(k => { if (z[k] !== undefined) platne[k] = z[k]; });
        const novy = Object.assign({}, zdroj, platne);
        delete novy.nasobekZeme;
        return novy;
    }
    ...
    function presunKategorii(nalezy) {
        ...
        nalezy.forEach(n => {
            ...
            const doZak = (n.kam === KAT_ZAKOUZLENO), zeZak = (n.radek.kat === KAT_ZAKOUZLENO);
            if (doZak && !zeZak) {
                zmeny.poznamka = [n.radek.poznamkaNejista || "",
                                  n.radek.poznamka ? ('"' + n.radek.poznamka + '"') : ""]
                    .filter(Boolean).join(" ");
                zmeny.poznamkaNejista = "";
            } else if (zeZak && !doZak) {
                zmeny.poznamka = vytahniUvozovky(n.radek.poznamka).poznamka;
                zmeny.poznamkaNejista = "";
            }
    ...
    function presunZeme(polozky) {
        ...
        (polozky || []).forEach(p => {
            ...
            const novy = p.smazat ? null : radekProZem(p.radek, p.zeme, {
                kouzlo: p.kouzlo, mo: p.mo, nasobek: p.nasobek });
            p.radek.zeme = p.radek.zeme.filter(z => normalizeText(z) !== jm);
            if (p.radek.moZeme) delete p.radek.moZeme[jm];
            if (novy) p.sekce.radky.push(novy);
    ...
    function zvysNasobek(nalezy) {
        presunZeme((nalezy || [])
            ...
            .filter(n => Math.abs(n.zbyva) && Math.abs(n.zbyva) !== ((n.radek && n.radek.nasobek) || 1))
            .map(n => ({ zeme: n.zeme, radek: n.radek, sekce: n.sekce,
                         nasobek: Math.abs(n.zbyva) })));
    }

    function smazZeZeme(nalezy) {
        presunZeme((nalezy || [])
            .map(n => ({ zeme: n.zeme, radek: n.radek, sekce: n.sekce, smazat: true })));
    }
    ...
    function nazevVeze(z) {
        const v = window.DarkElfUtils.parseTower(z && z.img_vez);
        return v ? v.cz : "žádná magická věž";
    }

    function skupinaMO(stav) {
        if (!stav || stav.mo == null) return "?";
        return "MO" + stav.mo + (stav.jistota === "presna" ? "" : "+");
    }

    function davkyProHrace(h, zeme, cast) {
        const mimo = {};
        (h.mimo || []).forEach(j => { mimo[normalizeText(j)] = true; });

        const hozeneNaZem = {};
        ((cast && cast.kouzla) || []).forEach(k => {
            const klic = String(k.zemeId);
            (hozeneNaZem[klic] = hozeneNaZem[klic] || []).push(k);
        });

        const jmenoHrace = normalizeText(h.jmeno);
        const skupiny = {}, poradi = [];
        let naMape = 0, mimoNalezeno = 0;

        (zeme || []).forEach(z => {
            if (!z || !z.hrac || normalizeText(z.hrac) !== jmenoHrace) return;
            naMape++;

            if (mimo[normalizeText(z.zeme)]) { mimoNalezeno++; return; }

            const stav = moProTyp(z, "cervene");
            const klic = skupinaMO(stav);
            if (!skupiny[klic]) {
                skupiny[klic] = { klic: klic, mo: stav.mo, jistota: stav.jistota, zeme: [] };
                poradi.push(klic);
            }

            const hozeno = (hozeneNaZem[String(z.id)] || []).map(k => {
                const moK = moProTyp(z, k.typ);
                return { kouzlo: k.kouzlo, kouzlic: k.kouzlic, sila: k.sila, typ: k.typ,
        ...
        return {
            jmeno: h.jmeno, vListu: h.pocet, naMape: naMape,
            mimo: (h.mimo || []).slice(), mimoNalezeno: mimoNalezeno,
            poznamka: h.poznamka || "",
            skupiny: poradi.map(k => {
                ...
                sk.hotove = sk.zeme.filter(x => x.hotovo).map(x => x.jmeno);
                return sk;
            })
        };
    }
    ...
    function nalozZeme(jmena, idx) {
        const ta = document.getElementById("textAreaMagic");
        if (!ta) return null;
        const text = (jmena || []).join(",");

        ta.value = text;
        ta.textContent = text;
        ["input", "keyup", "change"].forEach(ev => ta.dispatchEvent(new Event(ev, { bubbles: true })));
        return { pocet: (jmena || []).length, mapa: zvyrazniNaMape(jmena, idx) };
    }

    function zvyrazniNaMape(jmena, idx) {
        if (!idx) return null;
        const lands = [], chybi = [];
        (jmena || []).forEach(j => {
            const z = idx[normalizeText(j)];
            if (z && z.id != null) lands.push([String(z.id), z.zeme || j]);
            else chybi.push(j);
        });
        saveToStorageAndRefresh({ lands: lands });
        return { zeme: lands.map(l => l[1]), chybi: chybi };
    }

    function pripravKouzleni(r, idx) {
        const ta = document.getElementById("textAreaMagic");
        const sely = [];
        for (let i = 1; i <= 5; i++) {
    ...
    function nactiRozhodnuti() {
        try {
            const u = JSON.parse(localStorage.getItem(odecLigaKlic("jmena")));
    ...
    function ulozRozhodnuti(r) {
        try {
            localStorage.setItem(odecLigaKlic("jmena"),
                JSON.stringify({ datum: dnesniDatum(), klice: r }));
        } catch (e) {}
    }

    function nactiRozhodnutiOs() {
        try {
            const u = JSON.parse(localStorage.getItem(odecLigaKlic("osy")));
            if (!u || u.datum !== dnesniDatum()) return {};
            return u.klice || {};
        } catch (e) { return {}; }
    }
    function ulozRozhodnutiOs(r) {
        try {
            localStorage.setItem(odecLigaKlic("osy"),
                JSON.stringify({ datum: dnesniDatum(), klice: r }));
        } catch (e) {}
    }
    function klicOsy(n) {
    ...
    function nactiNaHraceSbaleno() {
        try { return localStorage.getItem(odecLigaKlic("nahrace")) === "1"; }
        catch (e) { return false; }
    }

    const OKNO_ID = "ml_nalezy_okno";

    const OKNO_ZNOVU_KLIC = "de_ml_okno_znovu";
    ...
    function glyfAkce(doc, znak, barva, titulek, naKlik, akce) {
        const a = doc.createElement(naKlik ? "a" : "span");
        a.style.cssText = "display:inline-block;width:14px;flex:0 0 14px;text-align:center;"
            + "font-size:12px;line-height:1;text-decoration:none;color:" + (barva || "#888") + ";"
            + (naKlik ? "cursor:pointer;" : "");
        a.textContent = naKlik ? znak : "";
        if (naKlik) {
            a.href = "#";
            a.title = titulek || "";
            if (akce) a.setAttribute("data-akce", akce);
            a.onclick = (ev) => { ev.preventDefault(); ev.stopPropagation(); naKlik(); };
        }
        return a;
    }

    const TITULEK_IGNOR = "Ignorovat do půlnoci — řádek se schová dolů do „Ignorováno“ "
        + "a přestane se počítat. Když se situace změní (jiná MO, jiná síla), ozve se znovu.";

    let ignorRozbaleno = false;

    function otevriOkno(nalezy, vstupEl, otazky, sluzby, sekce) {
        nalezy = nalezy || [];
        otazky = otazky || [];
        sluzby = sluzby || {};
        sekce = (sekce || []).filter(s => s && s.kresli);

        let scrollBylo = 0;
        try { const stary = najdiOkno(); if (stary) scrollBylo = stary.scrollTop || 0; } catch (e) { }

        zavriOkno();
        if (!nalezy.length && !otazky.length && !sekce.length) return;
        ...
        if (koren !== okno) {
            ...
            styl.textContent = ":host{all:initial;} div,span,a,b{" + zaklad + "}"
                + "input,button{" + zaklad + "font-family:Arial,sans-serif;font-size:11px;}"
                + "button{background:#332211;color:#ddd;border:1px solid #553311;"
                + "border-radius:2px;padding:3px 10px;cursor:pointer;}"
                + "input[type=text]{background:#241a0c;color:#ddd;border:1px solid #553311;}";
        ...
        krizek.href = "#";
        krizek.textContent = "✕";
        krizek.setAttribute("data-akce", "zavrit");
        ...
        const kVyreseni = otazky.length + nalezy.length;
        hlava.textContent = "Magic list" + (kVyreseni ? (" — " + kVyreseni + " k vyřešení") : "");
        ...
        const radekListu = (n) => {
            ...
            if (jdeOpravit) {
                duvod.href = "#";
                duvod.title = "Doplnit čárku a přechroustat znovu";
                duvod.onclick = (ev) => {
                    ev.preventDefault();
                    ev.stopPropagation();
                    sluzby.opravZem(n);
            ...
            if (sluzby.ignoruj && n.ignorDruh) {
                const obsah = doc.createElement("span");
                obsah.style.cssText = zaklad + "display:block;flex:1 1 auto;min-width:0;";
                while (r.firstChild) obsah.appendChild(r.firstChild);
                r.appendChild(obsah);
                r.style.display = "flex";
                r.style.alignItems = "baseline";
                r.style.gap = "4px";
                r.appendChild(jdeOpravit
                    ? glyfAkce(doc, "✓", "#99cc88", "Doplnit čárku a přechroustat znovu",
                               () => sluzby.opravZem(n), "zapis-ok")
                    : glyfAkce(doc, "", "", "", null));
                r.appendChild(glyfAkce(doc, "✗", "#cc7766", TITULEK_IGNOR,
                                       () => sluzby.ignoruj(n.ignorDruh, n), "ignor"));
            }
            return r;
        };
        ...
        const ovladaniOsy = (n) => {
            ...
            uvod.textContent = "Na " + o.zeme + " chci:";
            d.appendChild(uvod);

            const zkrList = S.zkratka(o.kouzlo);
            const kolik = Math.abs(o.zbyva);
            const nyni = (o.radek && o.radek.nasobek) || 1;

            const volba = (barva, kladny, farba) => {
                const jeVListu = ((o.pozadovano > 0) === kladny);

                if (jeVListu && o.vysledek === "nic") {
                    const sp = doc.createElement("span");
                    sp.style.cssText = zaklad + "display:inline;color:#888;";
                    sp.textContent = barva + " " + o.osa + " — nepomůže, letí " + kroku(o.dolu || 2) + " dolů";
                    sp.title = "Hra vyhodnotí žlutá kouzla před červenými a škála se nasytí na +1. "
                        + "Víc " + zkrList + " proto nepřidá nic.";
                    return sp;
                }

                const a = doc.createElement("a");
                a.href = "#";
                a.style.cssText = zaklad + "display:inline;text-decoration:none;color:" + farba + ";";
                const vListuText = (o.vysledek === "neutral")
                    ? ("1× " + zkrList + ", vyjde neutrál")
                    : (kolik + "× " + zkrList + (kolik <= nyni ? " (stačí, jak je psáno)" : ""));
                a.textContent = barva + " " + o.osa + " — "
                    + (jeVListu ? vListuText : (zkrList + " z listu smazat"));
                a.title = jeVListu
                    ? ((o.vysledek === "neutral")
                        ? ("Žlutá se vyhodnotí dřív a " + o.zdroje.join(", ") + " ji zruší — výsledek je neutrál. Víc "
                           + zkrList + " nepomůže.")
                        : (kolik > nyni
                            ? ("Násobek se zvedne na " + kolik + ", aby to překonalo " + o.zdroje.join(", "))
                            : ("Násobek z listu na to stačí — nic se nemění.")))
                    : ("Zem se vyjme z řádku — správně je to, co letí (" + o.zdroje.join(", ") + ")");
                a.onclick = (ev) => {
                    ev.preventDefault();
                    ev.stopPropagation();
                    if (jeVListu) { if (sluzby.zvysNasobek) sluzby.zvysNasobek(n); }
                    else if (sluzby.smazZem) sluzby.smazZem(n);
                };
                return a;
            };

            d.appendChild(volba("žlutou", true, "#ffcc66"));
            const nebo = doc.createElement("span");
            nebo.style.cssText = zaklad + "display:inline;color:#888;margin:0 6px;";
            nebo.textContent = "nebo";
            d.appendChild(nebo);
            d.appendChild(volba("červenou", false, "#ff8866"));
            return d;
        };
        ...
        sekce.forEach((sek, i) => {
            nadpisSekce(sek.nadpis, sek.barva || "#cc9944",
                        i > 0 || otazky.length > 0 || nalezy.length > 0);
            if (sek.napoveda) napovedaSekce(sek.napoveda);
            const host = doc.createElement("div");
            host.style.cssText = zaklad + "color:#ddd;line-height:1.35;";
            try { sek.kresli(host); } catch (e) { host.textContent = "nepovedlo se vykreslit"; }
            koren.appendChild(host);
        });

        const ignorovane = sluzby.ignorovane || [];
        if (ignorovane.length) {
            const hl = doc.createElement("a");
            hl.href = "#";
            hl.setAttribute("data-akce", "ignor-sbal");
            hl.style.cssText = zaklad + "display:block;margin-top:10px;padding-top:6px;"
                + "border-top:1px solid #553311;color:#888;text-decoration:none;cursor:pointer;";
            hl.textContent = (ignorRozbaleno ? "▾ " : "▸ ") + "Ignorováno do půlnoci ("
                + ignorovane.length + ")";
            hl.onclick = (ev) => {
                ev.preventDefault();
                ignorRozbaleno = !ignorRozbaleno;
                if (sluzby.prekresli) sluzby.prekresli();
            };
            koren.appendChild(hl);
            if (ignorRozbaleno) {
                ignorovane.forEach(x => {
                    const r = doc.createElement("div");
                    r.style.cssText = zaklad + "display:flex;align-items:baseline;gap:4px;"
                        + "margin:1px 0 1px 8px;color:#888;";
                    const t = doc.createElement("span");
                    t.style.cssText = zaklad + "display:block;flex:1 1 auto;min-width:0;";
                    t.textContent = x.popis;
                    r.appendChild(t);
                    r.appendChild(glyfAkce(doc, "↩", "#cc9944",
                        "Vrátit mezi nálezy — zase se bude počítat a hlásit.",
                        () => sluzby.vratIgnor(x.klic), "ignor-vrat"));
                    koren.appendChild(r);
                });
            }
        }
    ...
    function addPrehledML() {
        ...
        const stav = { data: null, sleva: { nejlepsi: 0, moje: 0, dostupne: false }, idx: null, idxId: null,
                       ...
                       rucni: null, presunuto: [],
                       zFormulare: null, revize: 0 };
        ...
        function nactiIgnor() {
            try {
                const u = JSON.parse(localStorage.getItem(odecLigaKlic("ml_ignor")));
                if (!u || u.datum !== dnesniDatum()) return {};
                return u.klice || {};
            } catch (e) { return {}; }
        }

        function ulozIgnor(klice) {
            try {
                localStorage.setItem(odecLigaKlic("ml_ignor"),
                    JSON.stringify({ datum: dnesniDatum(), klice: klice }));
            } catch (e) { }
        }

        function klicNalezu(druh, n) {
            const c = (x) => (x == null ? "" : String(x));
            const kl = (pole) => pole.map(c).join("|");
            switch (druh) {
                case "zapis":    return kl(["zapis", n.druh, n.cast, n.popis]);
                case "otazka":   return kl(["otazka", n.klic]);
                case "mo":       return kl(["mo", n.zeme, n.vListu, n.mapa]);
                case "neprosle": return kl(["neprosle", n.zeme, n.kouzlo, n.duvod, n.sila,
                                            n.pozadavek, n.mapa, n.pocet, n.potreba]);
                case "osy":      return kl(["osy", n.zeme, n.kouzlo, n.druh, n.pozadovano,
                                            n.dodano, (n.osy || [n.osa]).join("+")]);
                case "zak":      return kl(["zak", n.zeme, n.kouzlo, n.duvod, n.sila,
                                            n.pozadavek, n.pocet, n.potreba]);
                case "dupl":     return kl(["dupl", n.zeme, n.kouzlo,
                                            (n.kdo || []).slice().sort().join("+")]);
                case "hrac":     return kl(["hrac", n.jmeno, n.vListu, n.naMape]);
            }
            return druh + "|" + JSON.stringify(n);
        }

        function popisNalezu(druh, n) {
            const zk = (k) => window.DarkElfUtils.Spells.zkratka(k);
            const bezMO = (x) => (x && /^MO/.test(x)) ? x.replace(/^MO/, "") : "0";
            switch (druh) {
                case "zapis":    return (n.radek ? "ř. " + n.radek + ": " : "") + (n.cast || "")
                                        + " — " + (n.popis || "");
                case "otazka":   return "„" + n.text + "“ — hráč, nebo poznámka?";
                case "mo":       return n.zeme + " — MO(" + bezMO(n.vListu) + "→"
                                        + bezMO(n.navrh) + ")";
                case "neprosle": return n.zeme + " " + zk(n.kouzlo) + " — neprošlo, síla " + n.sila;
                case "osy":      return n.zeme + " " + zk(n.kouzlo) + " — škála";
                case "zak":      return n.zeme + " " + zk(n.kouzlo) + " — v Zakouzleno, ale nesedí";
                case "dupl":     return n.zeme + " " + zk(n.kouzlo) + " — poslali totéž ("
                                        + (n.kdo || []).join(", ") + ")";
                case "hrac":     return n.jmeno + " — v listu " + n.vListu + ", na mapě "
                                        + n.naMape;
            }
            return druh;
        }

        function nalezyKVyrizeni() {
            const ign = nactiIgnor();
            const out = { ignorovane: [] };
            const rozdel = (druh, pole) => {
                out[druh] = [];
                (pole || []).forEach(n => {
                    const k = klicNalezu(druh, n);
                    if (ign[k]) out.ignorovane.push({ klic: k, popis: popisNalezu(druh, n) });
                    else out[druh].push(n);
                });
            };
            const d = stav.data;
        ...
        function ignoruj(druh, n) {
            const k = nactiIgnor();
            k[klicNalezu(druh, n)] = true;
            ulozIgnor(k);
            vykresli();
            otevriOknoTed();
        }

        function vratIgnor(klic) {
            const k = nactiIgnor();
            delete k[klic];
            ulozIgnor(k);
            vykresli();
        ...
        function sAkcemi(d, naOk, okTitulek, naIgnor, akce) {
            const o = document.createElement("span");
            o.style.cssText = "display:block;flex:1 1 auto;min-width:0;";
            ...
            d.style.gap = "4px";
            d.appendChild(naOk
                ? glyfAkce(document, "✓", "#99cc88", okTitulek, naOk, akce ? akce + "-ok" : null)
                : glyfAkce(document, "", "", "", null));
            if (naIgnor) d.appendChild(glyfAkce(document, "✗", "#cc7766", TITULEK_IGNOR, naIgnor, "ignor"));
            return d;
        }

        function sVse(nadpis, kolik, titulek, naKlik, akce) {
            if (kolik < 2) return nadpis;
            const o = document.createElement("span");
            o.style.cssText = "display:block;flex:1 1 auto;min-width:0;";
            while (nadpis.firstChild) o.appendChild(nadpis.firstChild);
            nadpis.appendChild(o);
            nadpis.style.display = "flex";
            nadpis.style.alignItems = "baseline";
            nadpis.style.gap = "4px";
            const a = document.createElement("a");
            a.href = "#";
            a.textContent = "✓ vše";
            a.title = titulek;
            a.setAttribute("data-akce", akce);
            a.style.cssText = "flex:0 0 auto;color:#99cc88;text-decoration:none;cursor:pointer;"
                + "font-weight:normal;white-space:nowrap;";
            a.onclick = (ev) => { ev.preventDefault(); ev.stopPropagation(); naKlik(); };
            nadpis.appendChild(a);
            return nadpis;
        }

        function vsechnyRadky(data) {
            const out = [];
            ((data && data.sekce) || []).forEach(sek => sek.radky.forEach(r => out.push(r)));
            return out;
        }

        function sekceDoOkna() {
            const out = [];
            if (stav.idx && stav.data) {
                out.push({
                    nadpis: "⚖ Kontrola MO proti mapě", barva: "#cc9944",
                    napoveda: "✓ přepíše MO v listu, ✗ ignoruje do půlnoci. Do políčka "
                              + "se dá napsat vlastní hodnota, prázdné = použije se návrh.",
                    kresli: (host) => vykresliKontroluMO(host)
                });
            }
            if (stav.cast) {
                out.push({
                    nadpis: "⚔ Alianční kouzla", barva: "#cc9944",
            ...
            if (hracNalezy.length) {
                out.push({
                    nadpis: "👤 Soupiska hráčů", barva: "#aa88dd",
                    napoveda: "Počet zemí v listu nesedí s mapou. Opravit se to nedá — "
                              + "buď je mapa o pár minut starší, nebo hráč o zem přišel. "
                              + "✗ to schová do půlnoci.",
                    kresli: (host) => vykresliNesediHraci(host, hracNalezy)
                });
            }
            return out;
        }

        const PRVNI_VLNA = { radek: true };
        ...
        function vykresli(znovuParsovat) {
            ...
            poradiProVystup(stav.data).forEach(sek => {
                ...
                (hotovo ? sek.radky : rozpadPodleMO(sek.radky)).forEach(r => {
                    ...
                    if (!hotovo) {
                        ...
                        tdK.setAttribute("data-akce", "upravit");
                    ...
                    if (!hotovo && r.skMin && stav.mojeSK) {
                        const smim = (stav.mojeSK.max || 0) >= r.skMin;
                        tdK.style.cssText += smim ? "color:#aaffaa;font-weight:bold;" : "color:#888;";
                        tdK.title = (smim
                                ? ("Tohle můžeš seslat ty — tvoje SK " + stav.mojeSK.max
                                   + " dosáhne na " + r.skMin + ".")
                                : ("Chce to SK aspoň " + r.skMin + ", ty máš nejvýš "
                                   + stav.mojeSK.max + "."))
                            + "\nSíla seslání je náhodná, takže pod " + r.skMin
                            + " to zůstane v „Pro jistotu překouzlit s větší SK“."
                            + (tdK.title ? ("\n" + tdK.title) : "");
                    }
                    ...
                    const klicD = klicDavky(sek.nazev, klicRadku);
                    const vDavce = !!(stav.davka && stav.davka.poradi.indexOf(klicD) > -1);
                    const lzePricist = !!(stav.davka && !vDavce
                        && stav.davka.kouzlo === r.kouzlo.nazev
                        && stav.davka.nasobek === (r.nasobek || 1));

                    const pocet = document.createElement(hotovo ? "span" : "a");
                    pocet.textContent = (vDavce ? "✓ " : "") + r.zeme.length + " zemí";
                    if (!hotovo) {
                        pocet.href = "#";
                        pocet.setAttribute("data-akce", "radek");
                        pocet.title = vDavce
                            ? "Je v naložené dávce — klik ho zase odebere."
                            : (lzePricist
                                ? "Přičíst k naložené dávce (je to totéž kouzlo). Země se sečtou."
                                : "Naložit do kouzlení: kouzlo do K1…K5, země do herní buňky "
                                  + "a rovnou zaškrtnout na mapě.");
                        pocet.style.cssText = "color:" + (vDavce ? "#88bb66" : "#ddd")
                            + ";text-decoration:underline dotted;cursor:pointer;"
                            + (vDavce ? "font-weight:bold;" : "");
                        pocet.onclick = (ev) => {
                            ev.preventDefault();
                            const v = naklikniRadek(r, klicD);
                            ...
                            if (v.prazdna) {
                                stavKouzleni.textContent = "Dávka je prázdná.";
                                vykresli();
                                return;
                            }
                            if (v.chyba) { stavKouzleni.textContent = v.chyba; vykresli(); return; }

                            const chce = (r.nasobek || 1);
                            const zkracene = (v.kolik && v.kolik < chce)
                                ? ("Naloženo " + v.kolik + "× z " + chce
                                   + "× — hra má jen pět roletek. Zbytek zůstane v ML. ")
                                : "";
                            prekresliFormular((v.mapa && v.mapa.chybi.length)
                                ? (zkracene + "⚠ Mapa nezná: " + v.mapa.chybi.join(", ")
                                   + ". Dokud to v buňce zůstane, hra seslání odmítne.")
                                : (zkracene + "Zkontroluj a dej „Seslat na napsané země“."));
                            vykresli();
                        };
                    }
            ...
            if (rezervaZemi) {
                ...
                r2.title = "Řádky s požadavkem na nejsilnější seslání (_SKmax), "
                    + "které dostaly slabý hod. Sešli je znovu s maximální SK, "
                    + "až zbude mana a limit kouzel.";
            ...
            const nk = nalezyKVyrizeni();
            const cekaji = pocetKVyrizeni(nk);

            const ignN = nk.ignorovane.length;
            const ignText = ignN ? (" · " + ignN + " ignorováno") : "";
            if (cekaji) {
                const upoz = document.createElement("a");
                upoz.href = "#";
                upoz.style.cssText = "display:block;margin-top:6px;padding-top:4px;"
                    + "border-top:1px solid #553311;color:#ffcc66;font-weight:bold;"
                    + "text-decoration:underline dotted;";

                upoz.textContent = "⚠ " + cekaji + " "
                    + (cekaji === 1 ? "věc čeká" : (cekaji < 5 ? "věci čekají" : "věcí čeká"))
                    + " na vyřízení" + ignText + " — otevřít";
                upoz.onclick = (ev) => { ev.preventDefault(); otevriOknoTed(); };
                vystup.appendChild(upoz);
            } else if (stav.idx || stav.cast) {

                const odkaz = document.createElement("a");
                odkaz.href = "#";

                odkaz.style.cssText = "display:block;margin-top:6px;padding-top:4px;"
                    + "border-top:1px solid #553311;color:#cc9944;"
                    + "text-decoration:underline dotted;";
                odkaz.textContent = "kontroly a alianční přehled" + ignText + " — otevřít";
                odkaz.onclick = (ev) => { ev.preventDefault(); otevriOknoTed(); };
                vystup.appendChild(odkaz);
            }
            ...
            let pokrok = null;
            if (stav.data && stav.idx && stav.cast) {
                pokrok = {};
                jizZakouzlenoVPlanu(stav.data, stav.cast, stav.idx).forEach(n => {
                    const potreba = n.potreba || 1;
                    if (potreba < 2 || n.pocet <= 0 || n.pocet >= potreba) return;
                    const k = klicPokroku(n.radek, n.zeme);
                    if (pokrok[k] == null) pokrok[k] = n.pocet;
                });
            }

            const dopisky = (stav.zeme && stav.cast && (stav.data.hraci || []).length)
                ? dopiskyProHrace(stav.data.hraci, stav.zeme, stav.cast) : null;
            zpetnyText.value = exportML(stav.data, souhrnCeny, null, pokrok, dopisky);
        ...
        function vykresliNaHrace(kam) {
            const hraci = (stav.data && stav.data.hraci) || [];
            if (!hraci.length) return;

            const blok = document.createElement("div");
            blok.style.cssText = "margin:2px 0 6px 0;padding:2px 0 4px 5px;"
                + "border-left:2px solid #7755aa;";
            kam.appendChild(blok);

            const nadpis = document.createElement("a");
            nadpis.href = "#";
            nadpis.setAttribute("data-akce", "nahrace-sbal");
            nadpis.style.cssText = "display:block;text-decoration:none;font-weight:bold;color:#aa88dd;";
            nadpis.textContent = (stav.naHraceSbaleno ? "▸ " : "▾ ")
                + "Kouzlení na hráče · " + hraci.length;
            nadpis.onclick = (ev) => {
                ev.preventDefault();
                stav.naHraceSbaleno = !stav.naHraceSbaleno;
                try { localStorage.setItem(odecLigaKlic("nahrace"),
                                           stav.naHraceSbaleno ? "1" : "0"); } catch (e) { }
                vykresli();
            };
            blok.appendChild(nadpis);
            if (stav.naHraceSbaleno) return;

            if (!stav.zeme) {
                const cekam = document.createElement("div");
                cekam.style.cssText = "color:#888;";
                cekam.textContent = "země hráčů jsou v mapě — bez ní se dávky rozdělit nedají";
                blok.appendChild(cekam);
                return;
            }

            const hlaska = document.createElement("div");
            hlaska.id = "ml_hrace_hlaska";
            hlaska.style.cssText = "color:#88bb66;margin-top:3px;display:none;";

            const popisSeslani = (pole) => pole.map(x => x.kouzlo + " " + x.sila
                                                    + " (" + x.kouzlic + ")").join(", ");

            const jeVeVyberu = (hrac, klic) => !!stav.davkaHrace
                && stav.davkaHrace.hrac === hrac
                && stav.davkaHrace.klice.indexOf(klic) > -1;

            function zemeVyberu(v) {
                const d = stav.davkaHrace;
                if (!d || d.hrac !== v.jmeno) return [];
                const out = [];
                v.skupiny.forEach(sk => {
                    if (d.klice.indexOf(sk.klic) === -1) return;
                    sk.kNalozeni.forEach(z => { if (out.indexOf(z) === -1) out.push(z); });
                });
                return out;
            }

            function nalozVyber(v) {
                const zeme = zemeVyberu(v);
                if (!zeme.length) {
                    stav.davkaHrace = null;
                    nalozZeme([], stav.idx);
                    vykresli();
                    return;
                }
                const n = nalozZeme(zeme, stav.idx);
                vykresli();
                const h = document.getElementById("ml_hrace_hlaska");
                if (!h) return;
                h.style.display = "block";
                h.style.color = "#88bb66";
                const kolikSk = stav.davkaHrace.klice.length;
                h.textContent = n
                    ? ("Naloženo " + n.pocet + " zemí z " + kolikSk
                       + (kolikSk === 1 ? " dávky" : " dávek") + " (" + stav.davkaHrace.klice.join(", ")
                       + "). Vyber kouzlo v K1…K5 sám.")
                    : "Nenašel jsem herní buňku se zeměmi.";
            }

            function prepniSkupinu(hrac, klic) {
                const d = (stav.davkaHrace && stav.davkaHrace.hrac === hrac)
                    ? stav.davkaHrace : { hrac: hrac, klice: [] };
                const i = d.klice.indexOf(klic);
                if (i > -1) d.klice.splice(i, 1); else d.klice.push(klic);
                stav.davkaHrace = d.klice.length ? d : null;
                stav.davka = null;
                const v = davkyProHrace(hracPodleJmena(hrac), stav.zeme, stav.cast);
                nalozVyber(v);
            }

            const hracPodleJmena = (jm) => hraci.filter(h => h.jmeno === jm)[0] || null;

            const radekSkupiny = (v, sk, oddelit) => {
                const okraj = !oddelit ? ""
                    : (oddelit === "blok"
                ...
                klic.textContent = (vybrano ? "✓ " : "") + sk.klic;
                ...
                if (kolik) {
                    ...
                    hlavni.title = (vybrano
                        ? "Je ve výběru — klik ji zase odebere."
                        : "Přidá " + kolik + " zemí do výběru (dá se naklikat víc MO "
                          + "najednou). Kouzlo si vyber v roletce K1…K5 sám.");
                    hlavni.onclick = (ev) => {
                        ev.preventDefault();
                        prepniSkupinu(v.jmeno, sk.klic);
                    };
                } else {
                    hlavni.title = "Na každou zem téhle dávky dnes už něco prošlo.";
                }

                if (sk.hotove.length) {
                    pocet.style.cssText += "cursor:pointer;text-decoration:underline dotted;";
                    pocet.setAttribute("data-akce", "hotove");
                    pocet.title = "Naloží " + sk.hotove.length + " hotových zemí do buňky "
                        + "KE KONTROLE (zakouzlené: " + sk.hotove.join(", ") + ")."
                        + "\nNekouzli na ně znovu — jen se podívej a buňku pak vyprázdni.";
                    pocet.onclick = (ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
                        stav.davka = null;
                        stav.davkaHrace = null;
                        const n = nalozZeme(sk.hotove, stav.idx);
                        hlaska.style.display = "block";
                        hlaska.style.color = "#ffcc66";
                        hlaska.textContent = n
                            ? ("KONTROLA: naloženo " + n.pocet + " hotových zemí (" + sk.klic
                               + "). Tady už kouzlo prošlo — neposílej na ně další.")
                            : "Nenašel jsem herní buňku se zeměmi.";
                    };
                }
                tdM.appendChild(hlavni);

                const tdZ = document.createElement("td");
                tdZ.style.cssText = "vertical-align:top;padding:2px 0;color:#888;"
                    + "word-break:break-word;" + okraj;

                const kVypisu = sk.zeme.filter(z => !z.hotovo);
                kVypisu.forEach((z, i) => {
                    if (i) tdZ.appendChild(document.createTextNode(", "));
                    ...
                    if (z.zlute.length) {
                        ...
                        sp.style.cssText = "color:#ff8866;";
                        bublina.push("Odrazilo se: " + popisSeslani(z.hozeno)
                            + " — zem v dávce ZŮSTÁVÁ, odražené seslání není hotová práce.");
                    }
                    sp.title = bublina.join("\n");
                    tdZ.appendChild(sp);
                });
            ...
            const ramecekDavek = (v) => {
                const ram = document.createElement("div");

                ram.style.cssText = "margin:2px 0 4px 10px;padding:2px 4px;font-size:10px;"
                    + "border:1px solid #554477;border-radius:3px;"
                    + "background:rgba(119,85,170,0.10);";

                const lze = v.skupiny.filter(sk => sk.kNalozeni.length);
                if (lze.length > 1) {
                    const vse = document.createElement("a");
                    vse.href = "#";
                    vse.setAttribute("data-akce", "vse-hrace");
                    const vsechnyVybrane = !!stav.davkaHrace && stav.davkaHrace.hrac === v.jmeno
                        && lze.every(sk => jeVeVyberu(v.jmeno, sk.klic));
                    vse.style.cssText = "display:block;margin:1px 0 3px 0;color:#aa88dd;"
                        + "text-decoration:underline dotted;";
                    vse.textContent = vsechnyVybrane ? "✕ zrušit výběr" : "označit vše";
                    vse.title = vsechnyVybrane
                        ? "Vyprázdní výběr i herní buňku."
                        : "Naloží všechny dávky tohohle hráče naráz.";
                    vse.onclick = (ev) => {
                        ev.preventDefault();
                        stav.davka = null;
                        stav.davkaHrace = vsechnyVybrane
                            ? null
                            : { hrac: v.jmeno, klice: lze.map(sk => sk.klic) };
                        nalozVyber(v);
                    };
                    ram.appendChild(vse);
                }
                ...
                v.skupiny.forEach((sk, i) => {
                    ...
                    tab.appendChild(radekSkupiny(v, sk, oddelit));
                });
                ram.appendChild(tab);
                return ram;
            };
            ...
            hraci.forEach(h => {
                const v = davkyProHrace(h, stav.zeme, stav.cast);
                const rozbaleno = (stav.hracRozbalen === v.jmeno);

                const radek = document.createElement("div");
                radek.style.cssText = "margin-top:2px;";

                const a = document.createElement("a");
                a.href = "#";
                a.setAttribute("data-akce", "hrac");
                a.style.cssText = "text-decoration:none;color:#ddd;cursor:pointer;";
                a.textContent = (rozbaleno ? "▾ " : "▸ ") + v.jmeno;
                a.title = "Rozbalí jeho země po dávkách podle magické obrany."
                    + (v.poznamka ? ("\n\nPoznámka z listu: " + v.poznamka) : "")
                    + (v.mimo.length ? ("\n\nPod útokem (nekouzlí se): " + v.mimo.join(", ")) : "");
                a.onclick = (ev) => {
                    ev.preventDefault();
                    stav.hracRozbalen = rozbaleno ? null : v.jmeno;
                    vykresli();
                };
                radek.appendChild(a);
                ...
                info.textContent = " · " + v.naMape + (v.naMape === 1 ? " zem" : " zemí")
                ...
                if (v.vListu !== v.naMape) {
                    ...
                    varovani.textContent = " ⚠ v listu " + v.vListu;
                    varovani.title = "V listu je " + v.vListu + " zemí, na mapě jich vidím "
                        + v.naMape + ".";
                    radek.appendChild(varovani);
                }

                blok.appendChild(radek);
                if (rozbaleno) blok.appendChild(ramecekDavek(v));
            });

            blok.appendChild(hlaska);
        }

        function nesediPoctyHracu() {
            const hraci = (stav.data && stav.data.hraci) || [];
            if (!hraci.length || !stav.zeme) return [];
            const out = [];
            hraci.forEach(h => {

                const v = davkyProHrace(h, stav.zeme, null);
                if (h.pocet !== v.naMape) {
                    out.push({ jmeno: v.jmeno, vListu: h.pocet, naMape: v.naMape });
                }
            });
            return out;
        }
        ...
        function vykresliNesediHraci(kam, nalezy) {
            nalezy.forEach(n => {
                const d = document.createElement("div");
                d.style.cssText = "margin:1px 0;line-height:1.35;color:#ddd;";
                d.textContent = n.jmeno + " — v listu " + n.vListu + " zemí, na mapě "
                    + n.naMape;
                kam.appendChild(sAkcemi(d, null, "", () => ignoruj("hrac", n), "hrac"));
            });
        }

        function klicDavky(sekNazev, klicRadku) { return sekNazev + "||" + klicRadku; }

        function sada(pole) {
            const m = {};
            (pole || []).forEach(x => { m[normalizeText(x)] = true; });
            return m;
        }

        function zbyvaZDavky(d, vBunce) {
            if (!d) return null;
            const nase = sada(d.zeme);

            if (!vBunce.every(z => nase[normalizeText(z)])) return null;

            const je = sada(vBunce);

            if (d.zemeMapa) {
                const naMape = sada(d.zemeMapa);
                d.zeme.forEach(z => { if (!naMape[normalizeText(z)]) je[normalizeText(z)] = true; });
            }
            return d.zeme.filter(z => je[normalizeText(z)]);
        }

        function naklikniRadek(r, klic) {

            stav.davkaHrace = null;
            const ta = document.getElementById("textAreaMagic");
            const d = stav.davka;
            const vBunce = ta ? parseLands(ta.value) : [];

            const zbylo = (!!d && !!ta && d.kouzlo === r.kouzlo.nazev
                           && d.nasobek === (r.nasobek || 1))
                ? zbyvaZDavky(d, vBunce) : null;

            if (!zbylo) {
                stav.davka = { kouzlo: r.kouzlo.nazev, nasobek: (r.nasobek || 1),
                               poradi: [klic], casti: {}, text: "" };
                stav.davka.casti[klic] = r.zeme.slice();
            } else {

                const drzi = sada(zbylo);
                d.poradi = d.poradi.filter(k => {
                    d.casti[k] = (d.casti[k] || []).filter(z => drzi[normalizeText(z)]);
                    if (d.casti[k].length) return true;
                    delete d.casti[k];
                    return false;
                });

                if (d.poradi.indexOf(klic) > -1) {
                    d.poradi.splice(d.poradi.indexOf(klic), 1);
                    delete d.casti[klic];
                } else {
                    d.poradi.push(klic);
                    d.casti[klic] = r.zeme.slice();
                }
            }

            const dav = stav.davka;
            const zeme = [];
            dav.poradi.forEach(k => (dav.casti[k] || []).forEach(z => {
            ...
            if (!zeme.length) {
                stav.davka = null;
                return { prazdna: true, nalozeno: nalozZeme([], stav.idx) };
            }

            const v = pripravKouzleni({ kouzlo: r.kouzlo, nasobek: dav.nasobek, zeme: zeme },
                                      stav.idx);
            dav.text = ta ? ta.value : "";
            dav.zeme = zeme.slice();

            dav.zemeMapa = (v && v.mapa && v.mapa.zeme) ? v.mapa.zeme.slice() : null;
            return v;
        }
        ...
        function vykresliZFormulare() {
            ...
            pridat.onclick = (e) => {
                e.preventDefault();
                if (!stav.data) {
                    stav.data = { hlavicka: null, nejista: [], nezarazeno: [], hraci: [],
                                  sekce: [{ nazev: "", radky: [] }] };
                }
        ...
        function vykresliRucniMO() {
            ...
            (rucni.smazane || []).forEach((sm, i) => {
                ...
                zpet.onclick = (ev) => {
                    ev.preventDefault();
                    sm.sekce.radky.push(radekProZem(sm, sm.jmeno));
            ...
            volbaKouzla.setAttribute("data-akce", "volba-kouzla");
            ...
            rucni.zeme.forEach(jmeno => {
                ...
                zaskrt.type = "checkbox";
                zaskrt.style.cssText = "vertical-align:middle;margin:0 3px 0 0;";
                zaskrt.setAttribute("data-akce", "zem-vyber");
                ...
                pole.setAttribute("data-akce", "mo-zeme");
                ...
                krizek.onclick = (ev) => {
                    ...
                    const zav = (kde.radek.moZeme || {})[normalizeText(jmeno)];
                    rucni.smazane.push({
                        jmeno: jmeno, kouzlo: kde.radek.kouzlo, mo: kde.radek.mo,
                        poznamka: kde.radek.poznamka, kat: kde.radek.kat,
                        nasobek: kde.radek.nasobek, prio: kde.radek.prio,
                        skmax: !!kde.radek.skmax, skMin: kde.radek.skMin || 0,
                        neu: !!kde.radek.neu,
                        moZeme: zav ? { [normalizeText(jmeno)]: zav } : {},
                        sekce: kde.sekce });
            ...
            tlacZmena.setAttribute("data-akce", "pouzit");
            tlacZmena.title = "Přepíše kouzlo i počet seslání a zároveň MO u zemí, "
                + "kde je políčko vyplněné. Bez zaškrtnutí platí změna kouzla "
                + "pro celý řádek.";
            ...
            function obnovPopisky() {
                ...
                tlacZmena.textContent = "Použít " + (kolik ? ("u " + kolik + " zemí") : "u celého řádku");
            }
            obnovPopisky();
            ...
            tlacZmena.onclick = (ev) => {
                ...
                if (nalezy.length) opravMO(nalezy);
                ...
                ktereZeme().forEach(jmeno => {
                    ...
                    if ((!noveKouzlo || noveKouzlo.nazev === kde.radek.kouzlo.nazev)
                        && novyNasobek === (kde.radek.nasobek || 1)) return;
                    zmeny.push({ zeme: jmeno, radek: kde.radek, sekce: kde.sekce,
                                 kouzlo: noveKouzlo || kde.radek.kouzlo, nasobek: novyNasobek });
                });
                if (zmeny.length) presunZeme(zmeny);

                if (!nalezy.length && !zmeny.length) return;
                zmenaDat();
                stav.rucni = null;
                prekresliPoUprave();
            };
        ...
        function vykresliAliancni(kam) {
            ...
            if (neprosle.length) {
                ...
                neprosle.forEach(n => {
                    ...
                    d.title = n.skMin
                        ? ("list chce sílu aspoň " + n.skMin + ", sesláno " + n.sila
                           + " · seslal " + n.kouzlic)
                        : (n.duvod === "pocet")
                    ...
                    blok.appendChild(sAkcemi(d, null, null, () => ignoruj("neprosle", n)));
                });
            }
            ...
            if (osy.length) {
                ...
                osy.forEach(n => {
                    ...
                    if (n.druh === "mene") {
                        ...
                        co = "stačí " + Math.abs(n.zbyva) + "× místo "
                            + ((n.radek && n.radek.nasobek) || 1) + "×";
                    } else {
                        co = "netřeba";
                    }
                    ...
                    d.appendChild(t);
                    blok.appendChild(sAkcemi(d, null, null, () => ignoruj("osy", n)));

                    const klicNahrady = n.zeme + "|" + (n.radek ? stitekRadku(n.radek) : "");
                    if (nahradyVidene[klicNahrady] || !n.radek || !n.sekce) return;
                    nahradyVidene[klicNahrady] = true;
                    const zb = zbyvaKrokuNaOsach(n.radek, n.zeme, stav.cast, stav.idx);
                    if (!zb) return;
                    const moznosti = nahradniKouzla(n.radek, zb);

                    const zbyvaMax = Math.max.apply(null, Object.keys(zb.osy).map(o => zb.osy[o]));
                    const nasobekRadku = n.radek.nasobek || 1;

                    const cilovyNasobek = Math.max(1, zbyvaMax);
                    const lzeZmensit = !moznosti.length && cilovyNasobek < nasobekRadku;

                    const lzeSmazat = zbyvaMax === 0;
                    if (!moznosti.length && !lzeZmensit && !lzeSmazat) return;

                    const S = window.DarkElfUtils.Spells;
                    const nazevKouzel = (kouzla) => kouzla
                        .map(k => (k.nasobek > 1 ? k.nasobek + "×" : "") + S.zkratka(k.nazev)).join(" + ");
                    const puvodni = (nasobekRadku > 1 ? nasobekRadku + "×" : "") + zkr;

                    const v = radek("#bbbbbb");
                    v.style.marginLeft = "14px";
                    const chybi = Object.keys(zb.osy).filter(o => zb.osy[o] > 0)
                        .map(o => o + " " + zb.osy[o]).join(", ");
                    v.appendChild(document.createTextNode(chybi
                        ? ("zbývá " + chybi + " · místo " + puvodni + ": ")
                        : ("všechno už letí · " + puvodni + ": ")));
                    const odkaz = (text, titulek, akce, naKlik) => {
                        const a = document.createElement("a");
                        a.href = "#";
                        a.textContent = text;
                        a.title = titulek;
                        a.setAttribute("data-akce", akce);
                        a.style.cssText = "color:#ffcc66;text-decoration:underline dotted;cursor:pointer;";
                        a.onclick = (ev) => { ev.preventDefault(); ev.stopPropagation(); naKlik(); };
                        return a;
                    };
                    let prvni = true;
                    const oddel = () => {
                        if (!prvni) v.appendChild(document.createTextNode(" · "));
                        prvni = false;
                    };
                    moznosti.forEach(m => {
                        oddel();
                        v.appendChild(odkaz(nazevKouzel(m.kouzla) + " (" + m.cena + ")",
                            "V listu u " + n.zeme + " nahradí " + puvodni + " tímhle. MO, priorita i SK zůstanou.",
                            "skala-nahrada", () => {
                                nahradKouzloNaZemi(n.radek, n.sekce, n.zeme, m.kouzla);
                                poOpraveVOkne();
                            }));
                    });
                    if (lzeSmazat) {
                        oddel();
                        v.appendChild(odkaz("smazat zem",
                            n.zeme + " z řádku zmizí — na zem letí všechno, co řádek chtěl. "
                            + "Ostatní země řádku zůstanou.", "skala-smazat", () => {
                                nahradKouzloNaZemi(n.radek, n.sekce, n.zeme, []);
                                poOpraveVOkne();
                            }));
                    }
                    if (lzeZmensit) {
                        oddel();
                        v.appendChild(odkaz("zmenšit na " + cilovyNasobek + "×" + zkr,
                            "V listu u " + n.zeme + " sníží počet seslání. MO, priorita i SK zůstanou.",
                            "skala-zmensit", () => {
                                nahradKouzloNaZemi(n.radek, n.sekce, n.zeme,
                                    [{ nazev: n.radek.kouzlo.nazev, nasobek: cilovyNasobek }]);
                                poOpraveVOkne();
                            }));
                    }
                    oddel();
                    v.appendChild(odkaz("nechat " + puvodni,
                        "Nechat, jak je v listu. Hláška se schová do půlnoci.", "skala-nechat", () => {
                            const k = nactiIgnor();
                            osy.filter(x => x.zeme === n.zeme && x.radek === n.radek)
                               .forEach(x => { k[klicNalezu("osy", x)] = true; });
                            ulozIgnor(k);
                            vykresli();
                            otevriOknoTed();
                        }));
            ...
            if (vZakouzleno.length) {
                ...
                blok.appendChild(sVse(nadpis, vZakouzleno.length,
                    "Vrátit všech " + vZakouzleno.length + " zemí ze Zakouzleno zpátky do plánu",
                    () => { presunKategorii(vZakouzleno); poOpraveVOkne(); }, "zak-vse"));
                ...
                vZakouzleno.forEach(n => {
                    ...
                    d.appendChild(t);
                    blok.appendChild(sAkcemi(d,
                        () => { presunKategorii([n]); poOpraveVOkne(); },
                        "Vrátit tuhle zem ze Zakouzleno zpátky do plánu. Jméno kouzlícího se "
                            + "přitom ztratí — mimo Zakouzleno se v listu nedrží.",
                        () => ignoruj("zak", n), "zak"));
                });
            ...
            if (dupl.length) {
                ...
                blok.appendChild(d);
                dupl.forEach(x => {
                    const r = radek("#cc9944");
                    if (x.id != null) r.appendChild(odkazNaMapu(x.id));
                    const t = document.createElement("span");
                    t.textContent = x.zeme + " " + window.DarkElfUtils.Spells.zkratka(x.kouzlo)
                        + " — " + x.kdo.join(", ");
                    r.appendChild(t);
                    blok.appendChild(sAkcemi(r, null, null, () => ignoruj("dupl", x)));
                });
            }
        ...
        function vykresliKontroluMO(kam) {
            ...
            const ign = nactiIgnor();
            const nesedi = k.nesedi.filter(n => !ign[klicNalezu("mo", n)]);
            const ignorovano = k.nesedi.length - nesedi.length;
            const pole = [];

            const sVlastni = (n, vlastni) => {
                const rucne = vlastni.value.trim();
                if (rucne) {
                    const m = rucne.match(/^(\d+)(\+)?$/) || rucne.match(/^mo\s*(\d+)(\+)?$/i);
                    if (m) n.navrh = "MO" + m[1] + (m[2] || "");
                }
                return n;
            };
            ...
            shrnuti.textContent = "Kontrola MO: " + k.ok + " sedí"
                + (nesedi.length ? " · " + nesedi.length + " NESEDÍ" : "")
                + (ignorovano ? " · " + ignorovano + " ignorováno" : "")
                + (k.neovereno.length ? " · " + k.neovereno.length + " nelze ověřit" : "")
                + (k.neznameZeme.length ? " · " + k.neznameZeme.length + " zemí neznám" : "");
            blok.appendChild(sVse(shrnuti, nesedi.length,
                "Přepsat MO u všech " + nesedi.length + " zemí — kde je vyplněné políčko, platí to číslo",
                () => { opravMO(pole.map(v => sVlastni(v.nalez, v.vlastni))); poOpraveVOkne(); },
                "mo-vse"));
            ...
            nesedi.forEach(n => {
                ...
                vlastni.style.cssText = "width:26px;font-size:10px;margin-left:3px;"
                    + "background:transparent;color:#ddd;border:1px solid #553311;";
                pole.push({ nalez: n, vlastni: vlastni });
                ...
                blok.appendChild(sAkcemi(radek,
                    () => { opravMO([sVlastni(n, vlastni)]); poOpraveVOkne(); },
                    "Přepsat MO v listu — návrh " + n.navrh + ", nebo co je v políčku",
                    () => ignoruj("mo", n), "mo"));
            });
        ...
        async function nactiPodkladyOpravdu(vynutit) {
            ...
            try {
                ...
                if (cache && cache.zeme) {
                    zeme = cache.zeme;
                    idx = indexZemi(cache.zeme);
                    idxId = indexZemiPodleId(cache.zeme);
            ...
            let cast = null;
            try { cast = await window.DarkElfUtils.SpellsCastAPI.fetch(true); } catch (er) { cast = null; }
        ...
        async function obnovUlozene() {
            ...
            if (!list) return;
            obnovaBezela = true;
            const revizePred = stav.revize;
            await nactiPodklady();

            if (stav.revize !== revizePred) return;
            ...
            let znovu = false;
            try {
                znovu = sessionStorage.getItem(OKNO_ZNOVU_KLIC) === "1";
                sessionStorage.removeItem(OKNO_ZNOVU_KLIC);
            } catch (e) { }
            if (znovu) otevriOknoTed();
        }
        ...
        tlacitko.onclick = async (e) => {
            ...
            const revizePred = stav.revize;
            const radkyPred = vsechnyRadky(stav.data);

            await nactiPodklady();

            const pribyle = (stav.revize !== revizePred)
                ? vsechnyRadky(stav.data).filter(r => radkyPred.indexOf(r) === -1)
                : [];

            stav.data = parseML(vstup.value, { hraci: stav.hraci, zemeIdx: stav.idx, rozhodnuti: stav.rozhodnuti });
            if (pribyle.length) {
                const cil = stav.data.sekce[0] || (stav.data.sekce[0] = { nazev: "", radky: [] });
                pribyle.forEach(r => cil.radky.push(r));
            }
            presunHotove();
            vykresli();
        ...
        function hlidejBunku(ta) {
            if (!ta || typeof MutationObserver !== "function") return;
            const obs = new MutationObserver(() => {
                const obsah = ta.textContent;
                if (obsah === ta.value) return;
                ta.value = obsah;
                ["input", "change"].forEach(ev =>
                    ta.dispatchEvent(new Event(ev, { bubbles: true })));
            });
            obs.observe(ta, { childList: true, characterData: true, subtree: true });
        }
        ...
        window.addEventListener("pagehide", () => {
            try {
                if (najdiOkno()) sessionStorage.setItem(OKNO_ZNOVU_KLIC, "1");
            } catch (e) { }
            zavriOkno();
        });
    }
